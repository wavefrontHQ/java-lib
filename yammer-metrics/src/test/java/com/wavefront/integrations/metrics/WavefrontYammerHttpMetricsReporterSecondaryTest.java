...
public class WavefrontYammerHttpMetricsReporterSecondaryTest {

  private MetricsRegistry metricsRegistry;
  private WavefrontYammerHttpMetricsReporter wavefrontYammerHttpMetricsReporter;
  private HttpServer metricsServer, nullServer;
  ...
  private void innerSetUp(boolean prependGroupName, MetricTranslator metricTranslator,
    ...
    if (nullServer == null) {
      IOReactorConfig nullIOReactor = IOReactorConfig.custom().
          setIoThreadCount(5).
          build();
      nullServer = ServerBootstrap.bootstrap().
          setIOReactorConfig(nullIOReactor).
          setLocalAddress(InetAddress.getLoopbackAddress()).
          setListenerPort(0).
          setServerInfo("Test/1.1").
          registerHandler("*", new HttpAsyncRequestHandler<HttpRequest>() {
            ...
            @Override
            public void handle(HttpRequest httpRequest, HttpAsyncExchange httpAsyncExchange, HttpContext httpContext) throws HttpException, IOException {
              // Swallow and send an okay response
              httpAsyncExchange.submitResponse();
            }
          }).
          create();
    ...
    wavefrontYammerHttpMetricsReporter = new WavefrontYammerHttpMetricsReporter.Builder().
        ...
        withEndpoint(InetAddress.getLoopbackAddress().getHostAddress(), ((InetSocketAddress) nullServer.getEndpoint().getAddress()).getPort()).
        withSecondaryEndpoint(InetAddress.getLoopbackAddress().getHostAddress(), ((InetSocketAddress) metricsServer.getEndpoint().getAddress()).getPort()).
  ...
  public void testPlainHistogramWithClear() throws Exception {
    ...
    assertThat(inputMetrics, containsInAnyOrder(
        equalTo("\"myhisto.count\" 2.0 1485224035 source=\"test2\""),
        equalTo("\"myhisto.min\" 1.0 1485224035 source=\"test2\""),
        equalTo("\"myhisto.max\" 10.0 1485224035 source=\"test2\""),
        equalTo("\"myhisto.mean\" 5.5 1485224035 source=\"test2\""),
        equalTo("\"myhisto.sum\" 11.0 1485224035 source=\"test2\""),
        startsWith("\"myhisto.stddev\""),
        equalTo("\"myhisto.median\" 5.5 1485224035 source=\"test2\""),
        equalTo("\"myhisto.p75\" 10.0 1485224035 source=\"test2\""),
        equalTo("\"myhisto.p95\" 10.0 1485224035 source=\"test2\""),
        equalTo("\"myhisto.p99\" 10.0 1485224035 source=\"test2\""),
        equalTo("\"myhisto.p999\" 10.0 1485224035 source=\"test2\"")
    ));
    // Second run should clear data.
    runReporter();
    assertThat(inputMetrics, hasItem("\"myhisto.count\" 0.0 1485224035 source=\"test2\""));
  }
  ...
  public void testPlainMeter() throws Exception {
    ...
    runReporter();
    assertThat(inputMetrics, containsInAnyOrder(
        equalTo("\"mymeter.count\" 42.0 1485224035 source=\"test2\""),
  ...
  public void testTimerWithClear() throws Exception {
    ...
    assertThat(inputMetrics, containsInAnyOrder(
        equalTo("\"mytimer.rate.count\" 1.0 1485224035 source=\"test2\" \"foo\"=\"bar\""),
    ...
    runReporter();
    assertThat(inputMetrics, hasSize(15));
    assertThat(inputMetrics, hasItem("\"mytimer.rate.count\" 0.0 1485224035 source=\"test2\" \"foo\"=\"bar\""));
  }
  ...
  public void testPlainTimerWithoutClear() throws Exception {
    ...
    assertThat(inputMetrics, hasSize(15));
    assertThat(inputMetrics, containsInAnyOrder(
        equalTo("\"mytimer.rate.count\" 1.0 1485224035 source=\"test2\""),
        startsWith("\"mytimer.duration.min\""),
        startsWith("\"mytimer.duration.max\""),
        startsWith("\"mytimer.duration.mean\""),
        startsWith("\"mytimer.duration.sum\""),
  ...
  public void testPrependGroupName() throws Exception {
    ...
    assertThat(inputMetrics,
        containsInAnyOrder(
            equalTo("\"group.mycounter\" 2.0 1485224035 source=\"test2\" \"tag1\"=\"value1\" \"tag2\"=\"value2\""),
            equalTo("\"group2.myhisto.count\" 2.0 1485224035 source=\"test2\""),
            equalTo("\"group2.myhisto.min\" 1.0 1485224035 source=\"test2\""),
            equalTo("\"group2.myhisto.max\" 10.0 1485224035 source=\"test2\""),
            equalTo("\"group2.myhisto.mean\" 5.5 1485224035 source=\"test2\""),
            equalTo("\"group2.myhisto.sum\" 11.0 1485224035 source=\"test2\""),
            startsWith("\"group2.myhisto.stddev\""),
            equalTo("\"group2.myhisto.median\" 5.5 1485224035 source=\"test2\""),
            equalTo("\"group2.myhisto.p75\" 10.0 1485224035 source=\"test2\""),
            equalTo("\"group2.myhisto.p95\" 10.0 1485224035 source=\"test2\""),
            equalTo("\"group2.myhisto.p99\" 10.0 1485224035 source=\"test2\""),
            equalTo("\"group2.myhisto.p999\" 10.0 1485224035 source=\"test2\""),
            equalTo("!M " + timeBin +
                " #287 1.0 #69 5.0 #61 7.0 #58 8.0 #13 37.0 #7 66.0 #5 100.0 \"group3.myhisto\" source=\"test2\" " +
                "\"tag1\"=\"value1\" \"tag2\"=\"value2\"")));
  }
